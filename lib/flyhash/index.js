...
export class FlyIndex {
  ...
  constructor({ hasher, weights = {}, capacity = 1024, featureOpts = {} } = {}) {
    ...
    this.ids = [];
    this.meta = [];
    // id -> row, so add() doesn't scan the id list. With indexOf, reindexing
    // N records is O(N^2): at 100k rows that's ~5 billion comparisons and it
    // dominated indexing time.
    this.slots = new Map();
   ...
   */
  search(query, { limit = 10, minScore = 0, metric = 'cosine' } = {}) {
    const qTag = this.hasher.hash(this.#vector(query));
    return this.searchTag(qTag, { limit, minScore, metric });
  }

  searchTag(qTag, { limit = 10, minScore = 0, metric = 'cosine' } = {}) {
    ...
    for (let row = 0; row < size; row++) {
      ...
      // cosine     — "are these two texts alike?"  (deduplication)
      // containment — "is the query inside this record?"  (search box)
      //
      // A one-word query reaches ~18 cells; a full record reaches 102. Cosine
      // divides by sqrt(18*102) = 42.8, so a perfect match caps at 0.42 and a
      // realistic one lands near 0.11 — indistinguishable from noise. Dividing
      // by the smaller of the two counts asks the question a search box
      // actually means.
      const score = metric === 'containment'
        ? n / Math.min(qCount, counts[row])
        : n / Math.sqrt(qCount * counts[row]);
